...
#(parameter
    ...
    output wire [7:0]                    DataDimension,
    output wire [7:0]                    HiddenDimension,
...
//------------------------Parameter----------------------
localparam
    ADDR_DATAIN_DATA_0          = 6'h10,
    ADDR_DATAIN_DATA_1          = 6'h14,
    ADDR_DATAIN_CTRL            = 6'h18,
    ADDR_DATADIMENSION_DATA_0   = 6'h1c,
    ADDR_DATADIMENSION_CTRL     = 6'h20,
    ADDR_HIDDENDIMENSION_DATA_0 = 6'h24,
    ADDR_HIDDENDIMENSION_CTRL   = 6'h28,
    ADDR_DATAOUT_DATA_0         = 6'h2c,
    ADDR_DATAOUT_DATA_1         = 6'h30,
    ADDR_DATAOUT_CTRL           = 6'h34,
    WRIDLE                      = 2'd0,
    WRDATA                      = 2'd1,
    WRRESP                      = 2'd2,
    WRRESET                     = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [7:0]                    int_DataDimension = 'b0;
    reg  [7:0]                    int_HiddenDimension = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_DATADIMENSION_DATA_0: begin
                    rdata <= int_DataDimension[7:0];
                end
                ADDR_HIDDENDIMENSION_DATA_0: begin
                    rdata <= int_HiddenDimension[7:0];
                end
...
assign DataDimension   = int_DataDimension;
assign HiddenDimension = int_HiddenDimension;
...
// int_DataDimension[7:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_DataDimension[7:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DATADIMENSION_DATA_0)
            int_DataDimension[7:0] <= (WDATA[31:0] & wmask) | (int_DataDimension[7:0] & ~wmask);
    end
end

// int_HiddenDimension[7:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_HiddenDimension[7:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_HIDDENDIMENSION_DATA_0)
            int_HiddenDimension[7:0] <= (WDATA[31:0] & wmask) | (int_HiddenDimension[7:0] & ~wmask);
    end
end
